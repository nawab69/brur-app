...
export function UploadPayslip({ close }: { close?: any }) {
    ...
    return (
        <KeyboardAvoidingView>
            <Text className="font-['Inter-Medium'] text-main text-[18px] capitalize -mt-4 pb-3 mb-4 text-center border-b border-slate-300">Upload Playslip</Text>

            <View className="space-y-0">
                <TouchableOpacity className="flex items-start" onPress={() => setOpen(true)}>
                    <Text className="font-['Inter-Medium'] text-black text-xs bg-white translate-y-2 translate-x-7 z-10 px-2">Payment Date *</Text>
                    <TextInput editable={false} className="rounded-lg border-2 border-[#4A3FBC]/20 h-12 px-4 w-full text-black font-['Inter-SemiBold'] text-sm" value={date.toLocaleDateString()} />
                </TouchableOpacity>
                <View className="flex items-start">
                    <Text className="font-['Inter-Medium'] text-black text-xs bg-white translate-y-2 translate-x-7 z-10 px-2">AID *</Text>
                    <TextInput className="rounded-lg border-2 border-[#4A3FBC]/20 h-12 px-4 w-full text-black font-['Inter-SemiBold'] text-sm" value={aid} />
                </View>
            </View>
            ...
            <View className=''>
                <TouchableOpacity className='flex justify-center items-center space-y-2 border-2 border-dashed border-[#4A3FBC] p-4 mt-4' style={{ borderRadius: 10, borderStyle: 'dotted' }} onPress={handleChoosePhoto}>
                    ...
                    <Text className="font-['Inter-Medium'] text-black text-xs bg-white z-10 px-2">Upload Payslip</Text>
                </TouchableOpacity>
            </View>
